...
from Abb_robot.env.fetch import MujocoFetchEnv

MODEL_XML_PATH = os.path.join("fetch", "bin_pick.xml")


class MujocoBinPickEnv(MujocoFetchEnv, EzPickle):
    """
    ## Description
    ...
    | 3   | Actuator Control                                       | 0           | 1           | robot0:_suction_link                                            |       | position (N) |
    ...
    * `observation`: its value is an `ndarray` of shape `(25,)`. It consists of kinematic information of the block object and gripper. The elements of the array correspond to the following:
    ...
    | 9   | Joint displacement of the right gripper finger                                                                                        | -Inf   | Inf    |-                                      | robot0:suction_link                    | hinge    | position (m)             |
    | 10  | Not Used                                                                                                                              | -Inf   | Inf    |-                                      |-                                       |-         |                          |
    ...
    | 23  | Suction gripper finger linear velocity                                                                                                | -Inf   | Inf    |-                                      | robot0:suction_link                    | hinge    | velocity (m/s)           |
    | 24  | Suction Actuator State                                                                                                                | 0       | 1     |-                                      | adheregrip                             | -        |                          |
    ...
    * `desired_goal`: this key represents the final goal to be achieved. In this environment it is a 3-dimensional `ndarray`, `(3,)`, that consists of the three cartesian coordinates of the desired final block position `[x,y,z]`. In order for the robot to perform a pick and place trajectory, the goal position can be elevated over the table or on top of the table. The elements of the array are the following:
    Desire goal will change based on the objective current to the agent (if self.current task = 0 then the desired goal will be the block's position. If self.current task =1 then the desired goal will be the target0 listed in bin_pick.xml)
    ...
    * `achieved_goal`: this key represents the current state of the block, as if it would have achieved a goal. This is useful for goal orientated learning algorithms such as those that use [Hindsight Experience Replay](https://arxiv.org/abs/1707.01495) (HER). The value is an `ndarray` with shape `(3,)`. The elements of the array are the following:
    ...
    When the environment is reset the gripper is placed in the following global cartesian coordinates `(x,y,z) = [1.3419 0.7491 0.555] m`, and its orientation in quaternions is `(w,x,y,z) = [1.0, 0.0, 1.0, 0.0]`. The joint positions are computed by inverse kinematics internally by MuJoCo. The base of the robot will always be fixed at `(x,y,z) = [0.5 0.3 0]` in global coordinates.

    The block's position will be set to (x,y,z) = [1.25 0.7 0.27] m in global cartesian coordinates as its main placement site. To randomize the position there will be a -0.8 to 0.8 uniform value to be applied to the coordinates.
    ...
    env = gym.make('BinPick-v2', max_episode_steps=100)
    ```

    ## Version History

    * the environment depends on the newest [mujoco python bindings](https://mujoco.readthedocs.io/en/latest/python.html) maintained by the MuJoCo team in Deepmind.
    """
    ...
    def __init__(self, reward_type="dense", **kwargs):
        initial_qpos = {
            ...
            "robot0:slide1": 0.48,
            "robot0:slide2": 0.0,
            "object0:joint": [1.25, 0.7, 0.27, 1.0, 0.0, 0.0, 0.0],
        }
